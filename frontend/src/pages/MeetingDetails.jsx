            ...
            <header className="bg-[#1e293b] text-white px-6 py-4 flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center space-x-4 text-sm font-medium">
                    <span className="text-gray-300">
                        {meeting.date ? format(parseISO(meeting.date), 'MMMM d, yyyy') : ''}
                    </span>
            ...
            <div className="flex-1 flex overflow-hidden p-6 gap-6">
                ...
                <div className="w-1/2 flex flex-col overflow-y-auto pr-2 space-y-4 pb-20">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                        <h3 className="text-sm font-bold text-gray-900 mb-2">Meeting Objective</h3>
                        <p className="text-gray-700 text-sm leading-relaxed">{meeting.objective || 'N/A'}</p>
                    </div>

                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                        <h3 className="text-sm font-bold text-gray-900 mb-2">Executive Summary</h3>
                        <p className="text-gray-700 text-sm leading-relaxed whitespace-pre-line">{meeting.executive_summary || 'N/A'}</p>
                    </div>

                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                        <h3 className="text-sm font-bold text-gray-900 mb-3">Key Decisions</h3>
                        <ul className="space-y-2">
                            ...
                            )) : <li className="text-gray-500 text-sm">N/A</li>}
                        </ul>
                    </div>
                            ...
                            meeting.transcript.map((msg, index) => (
                                <div key={index} className="bg-gray-100/70 p-4 rounded-xl">
                                    <div className="flex justify-between items-baseline mb-1">
                                        <span className="font-semibold text-gray-900 text-sm">{msg.speaker_name || `Speaker ${msg.speaker_id}`}</span>
